    ...
    def addLinSolver(self, **args):
        ...
        if not args['name'] in self.tree['linear_solvers']['children']:
            self.tree['linear_solvers']['children'][
                args['name']] = self.populateTree('linear_solver', children={})
        ...
        if args['kind'] == "eigen":
            ...
            linear_solver['eigen']['children']['solver_type'] = self.populateTree(
                    'solver_type', text=args['solver_type'], children={})
            if "precon_type" in args:
                linear_solver['eigen']['children']['precon_type'] = self.populateTree(
                    'precon_type', text=args['precon_type'], children={})
            if "max_iteration_step" in args:
                linear_solver['eigen']['children']['max_iteration_step'] = self.populateTree(
                    'max_iteration_step', text=args['max_iteration_step'], children={})
            if "error_tolerance" in args:
                linear_solver['eigen']['children']['error_tolerance'] = self.populateTree(
                    'error_tolerance', text=args['error_tolerance'], children={})
            if "scaling" in args:
                linear_solver['eigen']['children']['scaling'] = self.populateTree(
            ...
            linear_solver['petsc']['children'][
                'prefix'] = self.populateTree('prefix', text=prefix, children={})
            ...
            linear_solver['petsc']['children'][
            'parameters'] = self.populateTree('parameters', text=string, children={})
